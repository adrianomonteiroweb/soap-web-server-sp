...
class DSGenerator {
  constructor() {
    this.keyFile = fs.readFileSync("./certificado/private.pem").toString();
    this.certFile = "./certificado/certificado.pem";
    const samplexml = fs.readFileSync("./xmls/samples.xml").toString();
    const outputFile = "./xmls/sined.xml";
...
class MyKeyInfo {
  ...
  getKeyInfo(key, prefix) {
    ...
    return (
      "<" +
      prefix +
      'X509Data><X509Certificate xmlns="http://www.w3.org/2000/09/xmldsig#">' +
      this.getKey() +
      '</X509Certificate><X509IssuerSerial xmlns="http://www.w3.org/2000/09/xmldsig#"><X509IssuerName>' +
      "Issuer Name" +
...
const generateDS = (samplexml, outputFile, keyFile, certFile) => {
  const { UniqueId, xml } = injectDynamicValues(samplexml);
  ...
  signAndValidateXml(xml, keyFile, certFile)
    .then((success) => {
      fs.writeFile(outputFile, success, (err, success) => {
        if (err) return err;
        console.log(
          `Signed XML Is In '${outputFile}' And UniqueId Is '${UniqueId}'.`
        );
      });
    })
    .catch((err) => {
...
const injectDynamicValues = (xml) => {
  ...
  for (key in keyValueObj) {
    doc.documentElement
      ...
      .item(0).firstChild.textContent = keyValueObj[key];
  }
